...
public class CalendarUtil {
    ...
    public static List<DateBean> getMonthDate(int year, int month, Map<String, String> map) {
        ...
        int week = com.example.androidqimo234.utils.SolarUtil.getFirstWeekOfMonth(year, month - 1);
        ...
        int lastMonthDays =com.example.androidqimo234.utils.SolarUtil.getMonthDays(lastYear, lastMonth);//上个月总天数

        int currentMonthDays = com.example.androidqimo234.utils.SolarUtil.getMonthDays(year, month);//当前月总天数
    ...
    private static DateBean initDateBean(int year, int month, int day, int type, Map<String, String> map) {
        ...
        if (map == null) {
            String[] temp = com.example.androidqimo234.utils.LunarUtil.solarToLunar(year, month, day);
        ...
        dateBean.setTerm(com.example.androidqimo234.utils.LunarUtil.getTermString(year, month - 1, day));

        if (type == 0) {
            dateBean.setSolarHoliday(com.example.androidqimo234.utils.SolarUtil.getSolarHoliday(year, month, day - 1));
        } else {
            dateBean.setSolarHoliday(com.example.androidqimo234.utils.SolarUtil.getSolarHoliday(year, month, day));
        }
        return dateBean;
    }
    ...
    public static int getMonthRows(int year, int month) {
        int items = com.example.androidqimo234.utils.SolarUtil.getFirstWeekOfMonth(year, month - 1) + com.example.androidqimo234.utils.SolarUtil.getMonthDays(year, month);
